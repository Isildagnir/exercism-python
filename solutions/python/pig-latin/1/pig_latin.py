...
def translate(text):
    ...
    for index_text, word in enumerate(text.split()):
        ...
        if word[cut_position] == 'q':
            translated_text += word[cut_position + 2:] + word[:cut_position + 2] + "ay"
        else:
            translated_text += word[cut_position:] + word[:cut_position] + "ay"
        if len(text.split()) > 1 and index_text < len(text.split()) - 1:
            translated_text += " "
